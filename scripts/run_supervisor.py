...
from common.adaptive_poll import AdaptivePollDelay
...
def submit_goal(base: str, goal: str, *, timeout: float) -> str:
    body: dict = {"goal": goal}
    if timeout != _conf.get("timeout", 3600.0):
...
def poll_goal(base: str, goal_id: str, *, timeout: float) -> dict:
    poll_delay = AdaptivePollDelay(
        min_delay=_conf.get("polling.min_delay", 1.0),
        max_delay=_conf.get("polling.max_delay", 30.0),
        backoff=_conf.get("polling.backoff", 1.5),
        burst_polls=_conf.get("polling.burst_polls", 2),
        cooldown_after=_conf.get("polling.cooldown_after", 20),
    )
...
def main() -> int:
    ...
    ap.add_argument(
        "--timeout", type=float,
        default=_conf.get("timeout", 3600.0),
